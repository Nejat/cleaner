...
pub fn list_empties<P: AsRef<Path>>(path: P, show_hidden: bool) {
    empties_handler("list", path, show_hidden, |_, msg| {
        println!("  - {msg}");
...
pub fn remove_empties<P: AsRef<Path>>(path: P, confirmed: bool, show_hidden: bool) {
    empties_handler("remove", path, show_hidden, move |empty, msg| {
...
fn empties_handler<F, P: AsRef<Path>>(action: &str, path: P, show_hidden: bool, handler: F)
where
